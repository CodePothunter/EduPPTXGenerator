...
edupptx/
  ...
  materials/
    ...
    # ── AI 图片素材复用库 (v3) ──
    ai_image_asset_db.py      # 复用 re-export shim (0 自有定义; 兼容旧 import, 逻辑已搬入 reuse/ 子包)
  ...
  reuse/                      # 复用库核心 (v3): ai_image_asset_db.py 8896 行单体重构出的 19 模块子包
    _decide.py                # 读路径主入口 find_reusable_ai_image_asset: policy 裁决 + 多库合池 + review 编排
    _retrieve.py              # 三路召回 (BM25+embedding+substring) + RRF 合池 (仅合池, 不参与裁决)
    _scoring.py               # BM25(真实IDF)/embedding/substring/policy 打分 (policy=(0.25kw+0.55emb)/0.80)
    _store.py / _build.py / _embedding.py     # 索引存储后端 / 建库 / 向量 npz sidecar
    _ingest.py / _materialize.py              # 后台入库队列编排 / 复用产物落地
    _keywords.py / _normalize.py / _gates.py  # 关键词路由 / 字段归一 / 语义·宽高比门控
    _review.py / _vlm.py                      # LLM 灰区审阅 / VLM 入库审查
    _constants.py _util.py _assets.py _backend.py _context.py _debug.py  # 常量单一来源/工具/资产/后端分发/上下文/调试
  postprocess/
...
- **卡片间距 20px** — 统一
- **13 种布局模式** (`LayoutHint`) — center_hero / vertical_list / bento_2col_equal / bento_2col_asymmetric / bento_3col / hero_top_cards_bottom / cards_top_hero_bottom / hero_with_microcards / mixed_grid / full_image / timeline / comparison / relation

### 自检审查
- **Phase 4 LLM 审阅 (实际行为)**: `svg_reviewer.review_and_fix_svg` 是**单次 LLM pass**——受 `_needs_llm_review` 成本门控（warning 全是已自动修好的小问题则跳过），**不渲染、不重试、无收敛循环**；LLM 若弄丢 `__IMAGE__` 占位符则整体回退审前 SVG。（注：这里不存在"生成→渲染→审查→再生成"的自动迭代环；cairosvg 只用于 embed 模式 PNG 兜底与图标渲染，不在 review 回路。）
- **手动/开发期 QA 技巧**: 用 `cairosvg.svg2png()` 把 SVG 直出 PNG 肉眼核查（不绕 LibreOffice/PDF）——这是人工检查手段，不是自动管线的一步。
