...
# 反 AI 套路硬规则。借鉴自成熟 PPT agent 工具的设计清单，挑出与教育 Bento
# 体系不冲突、且最能区分“专业版式 vs AI 糊版式”的条目。数值已换算到 1280×720
# 画布。与既有 references/ 规范互补，不重复。
_DESIGN_QUALITY_RULES = """
## 版式质量与反 AI 套路硬规则

- **标题下禁止装饰横线**：绝不在页面标题正下方画一条短下划线/装饰条来“点缀”标题——这是最典型的 AI 幻灯片特征。需要强调时用留白或背景色块区分，不要画线。
- **正文一律左对齐**：段落、要点、卡片说明文字全部左对齐；只有页面标题和单个大数字（KPI）允许居中。
- **相邻页禁止重复同一布局**：连续页面要在两列 / 卡片网格 / 大数字 / 关系图等模式间交替，避免每页视觉节奏雷同导致观众走神。
- **每页必须有非文字视觉元素**：至少一个形状 / 图示 / 图标 / 渐变色块 / 图表。纯标题+要点的页面等同于 Word 段落，不合格（公式页、引用页、单张汇总表除外）。
- **字号层级要拉开**：标题字号 ≥ 正文的 2 倍（本体系标题 ≥28px、正文 16–24px、标注 ≤14px）；层级不清就加大标题、压小标注。
- **两字体上限、一套主色板**：最多一种标题字体 + 一种正文字体；主色承担 60–70% 视觉权重，再加一辅色、一点缀色，正文内容里禁止出现 4 种以上颜色。
- **留白 ≥20%**：不要把画布填满；卡片区 x∈[50,1230]、卡间距 ≥20px，宁可少放卡片也不要缩字塞满。
- **单一视觉母题贯穿全篇**：选定一个标志性元素（如统一的圆角图片框 / 编号填充圆 / 单侧色带），每页都沿用，不要一页一个风格。
- **单个关键数字用大字号文字，不要用图表**：KPI 用 60–80px 的 `<text>` 呈现，数字控制在 5 字符内（`$`、`.` 算宽字符），过长则拆成“大数字 + 小号副标签”，不要为塞下单位而缩小字号。
- **图表选型对号入座**：类别比较用柱状图；时间序列用折线；占比 2–5 项用饼图（≥8 项不要用饼）；单一数值用大字号文字而非图表。
"""
...
def build_svg_system_prompt(
    ...
    parts.append(_DESIGN_QUALITY_RULES)
...
def build_svg_user_prompt(
    ...
    type_hints = {
        "cover": (
            ...
            "7. 标题文字绝不能直接压在插图/hero 图片上。若有插画素材，把它放在卡片的一侧或下部，"
            "为标题留出干净的纯色文字区；万不得已要让文字叠在图上时，必须在文字后加一层"
            "半透明圆角矩形蒙版（如白底 opacity 0.7 配深色字）保证可读性。\n"
        ),
        "toc": (
            "这是目录页。设计要求：\n"
