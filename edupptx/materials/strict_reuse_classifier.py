...
MATERIAL_CATEGORY_RULES_TEXT = (
    "## strict_reuse_group 分类规则（v8：单一主轴=替换不变性，去关键词清单补丁）\n"
    "\n"
    "全局原则：只看 query 字面描述，不参考 theme、subject、grade_norm、文件名、原始 strict_reuse_group 等元数据。"
    "按 C00→C01→C02→C03 顺序判断，命中即停。只允许输出以下 4 个主类 ID：\n"
    "C00_strict_text_problem_skip\n"
    "C01_irreplaceable_entity_event_action\n"
    "C02_generic_subject_object\n"
    "C03_scene_decor_container\n"
    "\n"
    "唯一主判据【替换不变性】：设想把这张图换成『另一张同样符合这段 query 的图』。要让复用后教学含义不被破坏，"
    "替换图最少必须和原图一致到哪个层级？层级越严越靠前：\n"
    "· 必须是同一张/精确内容，没有任何同类替换能保住含义 → C00；\n"
    "· 必须是同一个具名实体 → C01；\n"
    "· 同一类型/品类即可 → C02；\n"
    "· 同一氛围/场景即可 → C03。\n"
    "C00/C01 是『不可替换』，C02/C03 是『可在某层级替换』。下面四类只是这一条主判据的展开，不要额外引入关键词清单。\n"
    "\n"
    "C00_strict_text_problem_skip（精确不可替换载荷 — 跳过复用与入库）：\n"
    "画面里有必须逐一照搬才成立的特定载荷，换一处细节就教错。载荷的四种形态：\n"
    "① 字面符号：特定文字/汉字/拼音/词语/课文/题干/选项/公式/竖式/笔顺/部首/字源——教的就是这个符号本身；\n"
    "② 特定量值：具体数值/度量/角度/时刻等标注，那个量本身就是教学内容；\n"
    "③ 形状即信息：图形或曲线的特定形态编码了知识（某过程的特征曲线、对被教学的几何体/生物体逐项命名的结构标注），换一种形态就讲错；\n"
    "④ 受控对照/受控步骤：靠『特意设定的那一组对照项或操作步骤』才能得出结论（控制某量看另一量、特定推导或技术步骤的关键设定），换一组通用同类图结论就丢了。\n"
    "另含【可复现性下限】：当画面需同时照搬的并列特定元素过多，以致既难被复用精确匹配、也难被重新生成还原时，按 C00 跳过；凭『元素是否多到无法可靠复现』判断，不靠物体计数。\n"
    "不算载荷（→按主体走 C01/C02/C03）：代指任意对象的占位符号与同类图通用记号（如 a/b、//、⊥、主光轴/焦点）、空白脚手架（空网格/空表/空模板）、装饰性或可替换的短标签、附带的品牌/水印字。\n"
    "\n"
    "C01_irreplaceable_entity_event_action（唯一具名实体 或 强叙事时刻 — 严格匹配）：\n"
    "含义只由『那一个特定对象/时刻』承载，换成同类别的另一个就不再是它：\n"
    "① 唯一、可指认、不可替换的具名个体/角色/作品/地标/建筑/文献/文物——即便呈现为肖像、结构图或平面布局图，只要焦点是『就是它』即 C01；\n"
    "② 特定叙事时刻，其冲突/转折/强情绪本身是教学重点，换一张通用场景就丢了这一刻。\n"
    "反不变量：若『换一个同类仍服务同样复用』——类型名/品类、无具名焦点的泛对象、匿名人物或动物做通用日常活动——则不属 C01，按主体下沉 C02/C03。\n"
    "具名地标若其周边环境本身也可作氛围复用，额外输出 strict_reuse_secondary_group=C03_scene_decor_container；纯肖像/角色/文献/结构图不输出。\n"
    "\n"
    "C02_generic_subject_object（通用类型对象 — 类型级匹配）：\n"
    "画面核心是可辨识但通用、匿名、可替换的主体/对象/工具/器材，或匿名主体的通用动作；换任意一张同类型的图含义都成立。\n"
    "涵盖：通用器材即便静态标注标准部件名、单步通用操作、通用姿势或手势、单一通用概念/原理/定义图、可替换的通用对照、离散的单个自然主体。\n"
    "与 C00 的界：教的是『这一类』，没有必须照搬的字符/量值/形态/受控设定；与 C01 的界：主体匿名可替换，不是某个具名个体。\n"
    "\n"
    "C03_scene_decor_container（场景装饰容器 — 语境级匹配）：\n"
    "画面无主体动作焦点，核心是纯景观/远景/氛围/天气/装饰/边框/留白容器/版式占位；换任意一张同氛围的图都成立。\n"
    "匿名小人物仅作点缀融入远景才留 C03；一旦某主体或人物/动物的活动成为重点，按主体走 C02 或 C01，不因『有场景』就判 C03。\n"
    "\n"
    "C00↔C01 撞车：画面核心是逐字符号/量值载荷 → C00 优先；画面核心是某个具名实体本身（即便含字、呈现为结构图）→ C01 优先。\n"
)
