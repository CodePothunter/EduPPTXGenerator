...
MATERIAL_CATEGORY_RULES_TEXT = (
    ...
    "空白田字格模板/空烧杯线稿）无任何要复现的载荷——无论 query 是否带轴刻度/网格标号数字，都按主体走 C02/C03，不进 C00；"
    "轴刻度/网格标号/通用量具印着的刻度/标号、装饰文字、可替换短标签都不算要逐字复现的载荷。\n"
    "\n"
    "1. C01_irreplaceable_entity_event_action（唯一不可替换具名个体 或 复杂动作叙事 — 严格匹配）：\n"
    "C01 唯一性不变量：仅限唯一、可指认、不可替换的具名个体/作品/事件/具体地标。判据『换一件同类是否仍服务同样复用』——"
    "是→不属 C01（类型/品类名如先秦青铜簋＝一类青铜器、某种瀑布下放 C02，无具名焦点泛景观下放 C03）；"
    "否（必须就是这一件/这一人，如后母戊鼎、朱自清、卢沟桥、黄文秀日记）→C01。\n"
    "满足任一即归 C01：唯一具名/特定人物（含肖像/照片/塑像，如李白、鲁迅、黄文秀）；具名/特定角色"
    ...
    "教通用动作=C02，教特定拼音/字符符号=C00；通用物上的品牌/标识/标签字只是附带标记、不升级类别"
    "（米其林轮胎、带水印的手电筒光束仍 C02）——但当可读文字承载具名实体身份（封面题名、文档/日记页）时，"
    "按具名实体优先归 C01，不留在 C02 也不降 C00；C00 仅留给无具名实体的纯文字/题目载荷。自然物按离散主体与整体景观二分：可单独指代、可替换的"
    "离散主体（一棵树/一只鹤）归 C02。\n"
    "\n"
