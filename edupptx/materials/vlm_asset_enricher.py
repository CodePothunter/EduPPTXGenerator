...
额外复用分组判断：
- 同时判断图片应该进入哪个复用检索组，只能输出 visual_reuse_group="general_reuse" 或 "content_reuse"。
- 判别基于教学类别，不基于课文绑定深度。
- content_reuse 只覆盖以下五类「图像本身就是题目/字词/段落」的教学素材：
  (1) 语文识字——画面就是汉字字形本身（汉字识写示范、汉字演变图含具体古文字/楷书字、米字格/田字格中的具体汉字、生字卡片）。
  (2) 拼音——画面就是拼音字母/声调/拼音字词卡片本身。
  (3) 课文段落——课文原文文字直接印在画面上（不是描绘课文情节的插画，是把课文段落作为可读文字呈现）。
  (4) 数学题目——画面就是算式/应用题题面/看图列式/公式方程/带数字的数据表/统计图/票据本身。
  (5) 物理题目——画面就是物理示意图带具体标号（MN、AB、∠1、F=ma）/电路图带元件标号/受力分析/实验装置带标号本身。
- general_reuse 覆盖其余一切情形。关键：以下情形虽然「绑定课文」但仍是 general_reuse：
  课文情节场景插画（即便描绘小蝌蚪与青蛙、北海公园秋景、母亲推轮椅等具体课文场景）、
  课文角色或具名个体画像（即便是史铁生、贾谊这类历史人物的肖像，只要画面上没印必须匹配的文字）、
  故事道具或核心意象的描绘（菊花、荷叶、轮椅作为画面主体而非作为有文字的教学卡片）、
  通用教具实物（显微镜、三角尺、七巧板）、装饰插画、空白卡片、课堂活动场景、人物动作和情绪。
- 判别步骤：先观察画面，问自己一句「画面本身是不是字词/段落/题目这种『可读教学内容』？」
  是 → content_reuse；只是「某主题的插画/场景/角色画像」→ general_reuse。
- 原因：「叙事一致性」（课文里讲谁、讲哪里、讲什么情节）由课文文字、标题、caption 承担，不是图像的工作；
  图像层只在「图像本体即可读教学内容」时才需要逐像素匹配。
- visual_reuse_confidence 表示你对该分组判断的置信度，0-1。
- visual_reuse_reason 必须采用以下两种格式之一，不允许写"核心角色""故事绑定""教学意象"等泛化措辞：
  * 若判 content_reuse："属于<五类之一>：<画面上具体字词/算式/题面/标号片段>"（例："属于语文识字：汉字'兔'的字形演变"）。
  * 若判 general_reuse："属于场景/角色/教具/装饰插画：<画面主体描述>"（例："属于课文场景插画：小蝌蚪与青蛙团聚"）。
- 输出 JSON 需要额外包含：
  "visual_reuse_group": "general_reuse|content_reuse",
...
7. 不输出 core_keywords、semantic_aliases、query_aliases、constraints 或结构之外字段。"""
...
def _optional_strict_reuse_group(value: Any) -> str:
    ...
    return group if group in STRICT_REUSE_GROUPS else ""
...
def _apply_redescription(asset: dict[str, Any], payload: dict[str, Any]) -> None:
    ...
    content_prompt = _clean_text(payload.get("content_prompt"))
    if content_prompt:
        asset["content_prompt"] = content_prompt
    detail_prompt = _clean_text(payload.get("detail_prompt"))
    if detail_prompt:
        asset["detail_prompt"] = detail_prompt
...
def _build_debug_review_record(review_record: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "content_prompt": _clean_text(review_record.get("content_prompt")),
