...
VLM_REDESCRIBE_SYSTEM_PROMPT = """你是教学课件素材库的图片标注助手。给定一张图片和原始元数据，只根据图片实际内容生成可复用 caption，并据此判断素材分类，只返回严格 JSON。
...
输出 JSON 结构：
{
  "caption": "按 CAPTION_RULE 生成的短中文短语或短句",
  "context_summary": "一句 20-40 个汉字的短句，描述画面内容和页面功能",
  "teaching_intent": "该图服务的教学动作或学习目标",
  "general": false,
  "strict_reuse_group": "C00_strict_text_problem_skip|C01_language_glyph_visual|C02_structure_diagram_visual|C03_irreplaceable_entity_event_action|C04_generic_subject_object|C05_scene_decor_container",
  "strict_reuse_reason": "依据 caption 形态或具体观察给出分类理由"
}

规则：
1. 先生成 caption，再只依据 caption 判断 strict_reuse_group。
2. strict_reuse_group 只能使用上述 6 个 C00-C05 ID。
3. strict_reuse_reason 必须引用 caption 形态或图片中的具体观察，不得使用“核心角色/故事绑定”等泛化词。
4. general 必须是布尔值，表示当前图片本身是否可跨语文、数学、物理通用复用。
5. 不输出 core_keywords、semantic_aliases、query_aliases、constraints 或结构之外字段。"""
...
def _asset_metadata_for_vlm(asset: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in _METADATA_FIELDS:
        value = _asset_caption(asset) if field == "caption" else asset.get(field)
        if value in (None, "", [], {}):
            continue
...
def _apply_redescription(asset: dict[str, Any], payload: dict[str, Any]) -> None:
    ...
    strict_reuse_group = normalize_strict_reuse_group(payload.get("strict_reuse_group"), default="")
    if strict_reuse_group:
        asset["strict_reuse_group"] = strict_reuse_group
    strict_reuse_reason = _clean_text(payload.get("strict_reuse_reason"))
    if strict_reuse_reason:
        asset["strict_reuse_reason"] = strict_reuse_reason
    strict_reuse_confidence = _to_score(payload.get("strict_reuse_confidence"))
    if strict_reuse_confidence is not None:
        asset["strict_reuse_confidence"] = round(strict_reuse_confidence, 4)
