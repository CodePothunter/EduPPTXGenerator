...
4. 历史/文化题材身份：画面主体是历史文物/考古器物（青铜器、瓷器、编钟、出土器物）、
   ...
   · 通用器物中性：显微镜、放大镜、直尺等通用器材单体实物不触发 false。
5. 整体场景题材：整体山水/风景/园林/场景/氛围图（任何媒介：国画、水彩、写实皆算）→ false，
   偏语文/古典 register，放数学/物理页违和。注意：国画/水墨笔法本身不是触发器——
   离散单主体（单个花/鸟/竹/石/动物/工具/物件/卡通角色/装饰边框/空白容器）即便用国画/水墨风格，仍 general=true。

general=true（仅在以上都不命中时）：领域中性的离散通用视觉元素，放在任意一科都不违和。
例如通用卡通角色/人物、单个动植物、日常物件、通用工具、装饰/边框/空白容器/对话气泡——即便用国画/水墨风格。

示例（演示规则，不是逐例补丁）：
- 抱着橡果的卡通松鼠 / 双目光学显微镜实物 / 国画风格单只飞鸟 / 手握黄绿色直尺 / 空白装饰文本框 → true
- 水墨山水风景插画 / 水彩山峦 / 西湖晴天湖景 → false（整体场景题材，偏语文 register）
- 青铜鼎 → false（历史文物题材）
