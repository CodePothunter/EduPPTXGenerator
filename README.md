...
- 可选：`EDUPPTX_LLM_MAX_WORKERS`、`EDUPPTX_REUSE_COVERAGE_LOG`、`EDUPPTX_AI_IMAGE_REUSE_DEBUG_MODE`、`EDUPPTX_AI_IMAGE_EMBEDDING_BUILD_BATCH_SIZE`。
...
> **显式 `-s` 锁定配色**：传入 `edu_academic` / `edu_warm` / `edu_minimal` / `edu_tech` 时，该主题的整套配色会**覆盖**按学科/学段自动推断的调色板。
> 不传 `-s`（即默认 `edu_emerald`）时按主题自动选色——因此 `edu_emerald` 不参与锁定，等同"自动路由"。
> 注：`-s` 只锁**颜色**；版式模板族仍按学科/学段自动路由。颜色在 `gen` 阶段固化进 `plan.json`，故 `render` 阶段改 `-s` 不再生效（应改 `plan.json` / `DESIGN.md`）。
