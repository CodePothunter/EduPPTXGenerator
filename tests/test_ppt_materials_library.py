...
from edupptx.materials.reuse_policy import normalize_constraints
...
def _write_minimal_pptx(pptx_path: Path, image_path: Path, *, slide_text: str = "Lesson image page") -> None:
...
def test_build_ppt_image_materials_library_writes_match_index_and_embedding_sidecars(tmp_path, monkeypatch):
    ...
    index = json.loads((library_dir / "strict_reuse_indexes" / "general_reuse.json").read_text(encoding="utf-8"))
...
def test_ppt_near_duplicate_character_assets_keep_best_display_image(tmp_path):
    ...
    base = {
        ...
        "constraints": [
            {"kind": "entity", "subtype": "generic_class", "value": "girl", "importance": 1},
            {"kind": "action", "subtype": "generic_motion", "value": "speaking", "importance": 1},
        ],
        "core_keywords": ["girl", "speaking"],
        "semantic_aliases": {},
...
def test_build_ppt_image_materials_library_uses_vlm_metadata(tmp_path):
                        ...
                        "constraints": [
                            {
                                ...
                                "value": "blue rectangle",
                                "importance": 1,
                                "confidence": 0.9,
                                "evidence": "content_prompt describes a blue rectangle",
                                "reason": "The rectangle is the teaching carrier",
                            }
                        ],
    ...
    assert asset["padding_capacity"] == "low"
    ...
    assert "blue rectangle" in asset["core_keywords"]
    assert "visual support" in asset["core_keywords"]
    assert "should not be used from vlm" not in asset["core_keywords"]
    assert asset["constraints"][0]["value"] == "blue rectangle"
    assert asset["constraints"][0]["subtype"] == "teaching_carrier"
    assert asset["context_summary_keywords"] == ["visual support"]
    assert asset["topic_refs"] == ["lesson"]
    assert asset["duplicate_asset_ids"] == []
...
def test_ppt_annotation_normalization_keeps_only_vlm_semantics():
    annotation = {
        ...
        # legacy nested shape on input — back-compat read should still extract
        # the inner value and write canonical top-level on output.
        "transform_advice": {"padding_capacity": "LOW"},
        "asset_category": "content_specific",
        "constraints": [
            {"kind": "text", "subtype": "teaching_content", "value": "傻", "importance": 2},
            {"kind": "entity", "subtype": "named_individual", "value": "刷子李", "importance": 2},
        ],
        "core_keywords": ["语文", "五年级", "五年级语文", "《刷子李》", "田字格", "傻"],
        "semantic_aliases": {},
        "visible_text": ["傻"],
...
def test_ppt_annotation_normalization_falls_back_when_detail_prompt_missing():
    ...
    assert normalized["padding_capacity"] == ""
...
def test_match_index_keeps_distinct_text_teaching_cards(tmp_path):
    ...
    base = {
        ...
        "context_summary_keywords": ["字词学习", "生字教学"],
        "asset_category": "content_specific",
        "semantic_aliases": {},
        "duplicate_asset_ids": [],
    }
        ...
        "assets": [
            {
                **base,
                ...
                "content_prompt": "生字教学卡片，展示汉字“傻”的拼音、田字格写法、笔画数和部首，配有发音按钮和笔顺播放选项",
                "core_keywords": ["傻", "拼音", "田字格", "部首"],
                "constraints": [
                    {"kind": "text", "subtype": "teaching_content", "value": "傻", "importance": 2},
                    {"kind": "text", "subtype": "teaching_content", "value": "shǎ", "importance": 2},
                ],
            },
            {
                **base,
                ...
                "content_prompt": "生字教学卡片，展示汉字“堪”的拼音、田字格写法、笔画数和部首，配有发音按钮和笔顺播放选项",
                "core_keywords": ["堪", "拼音", "田字格", "部首"],
                "constraints": [
                    {"kind": "text", "subtype": "teaching_content", "value": "堪", "importance": 2},
                    {"kind": "text", "subtype": "teaching_content", "value": "kān", "importance": 2},
                ],
            },
        ],
    }
...
def test_constraint_normalization_collapses_same_kind_value_and_keeps_highest_importance():
    ...
    black_gown = [item for item in constraints if item["kind"] == "object" and item["value"] == "黑色长衫"]
    assert len(black_gown) == 1
    assert black_gown[0]["importance"] == 1
    assert black_gown[0]["subtype"] == "scene_prop"
    assert black_gown[0]["confidence"] == 0.95
    assert [item["value"] for item in constraints] == ["黑色长衫", "刷墙匠人"]
