...
可选 DESIGN.md 路径（由 `EDUPPTX_VISUAL_PLANNER_FORMAT=design_md` 开启）：除了 `VisualPlan` JSON 之外，还会让 LLM 输出一份 8 段 markdown DESIGN.md 写入 `session_dir/DESIGN.md`，作为可读、可改、可 diff 的视觉系统快照。当 `palette_hint` 已确定（来自模板路由）时走"prose-only"路径——颜色锁定、LLM 仅生成 prose；无 hint 时让 LLM 自由生成完整 DESIGN.md。LLM 全失败时 `_fallback_design_md` 兜底，不阻塞 Phase 3。

DESIGN.md 中的 Components / Do's & Don'ts / Elevation / Shapes 四段 prose 与 typography（硬字号/字体）会被 `build_phase3_constraints` 抽出、`{colors.xxx}` 解析为 hex 后注入 SVG system prompt 末段，驱动 Phase 3 输出。`edupptx render` 也会读取 `session_dir/DESIGN.md` 并应用——用户手编辑后重跑 render 即生效；DESIGN.md 加载后立即过 `resolve_style` lint，broken-ref 触发降级（不注入但不阻塞生成）。
