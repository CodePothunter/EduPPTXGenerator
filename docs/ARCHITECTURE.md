...
> 更新日期：2026-06-17
> 基准：`master`（含 Phase A 复用层重构 + R2b substring 修复 + R3 真实 IDF + 本轮 Phase 4 P0/P1 修复：裸 `<` 标签成形转义、overlap 后文字再夹、sanitizer 共用硬化 parser、review 后再校验）。
...
**一个关键结构事实**：代码体量与测试投入的大头压在 **AI 图片素材复用库（v3）**——让生成过的图片能跨会话复用，省钱省时。它原本是一个 8896 行的单体文件，现已**重构成 `edupptx/reuse/` 子包（19 个模块），原文件留作 1126 行的公共门面**（0 自有定义、纯 re-export，全仓仍经它 import；不是临时兼容层，见 §3.1）。理解这一点是理解整个项目当前状态的关键：主线 SVG 管线成熟稳定但测试单薄，复用库代码量大、迭代频繁、测试扎实。
...
| `materials/ai_image_asset_db.py` | 1126 | **复用库唯一公共门面**（0 自有定义的 re-export；23 个源文件经它 import，0 个直接 import `reuse/`，见 §3.1） |
...
1. **确定性修复 `validate_and_fix`**（`svg_validator.py`，2200 行，~25 类规则）：先做字符级预清洗（HTML 实体转数字实体、裸 `&` 转义；裸 `<` 用**标签成形 lookahead** 转义——只有"下一个 `<` 之前有 `>` 闭合"才算标签，于是 `n<k` / `0<x<1` 这类紧贴字母的数学不等式被当字面文本而非误判成标签开头，旧版的"只看下一个字符"会让 `n<k` 整页解析失败、静默退回原文跳过全部修复）。再用一个**写死安全配置的解析器**（`_SAFE_PARSER`，`resolve_entities=False, no_network=True`，**这就是防 XXE 的关键**——禁实体展开、禁联网取 DTD，挡住恶意 SVG 读本地文件）。然后按**硬依赖顺序**跑规则：必须先 `_wrap_long_text`（长文折成多行）→ 再 `_clamp_boundaries`（坐标夹进安全区 x∈[50,1230]、底边≤660/720）→ 再 `_fix_text_overlaps`（重叠文字往下推）。顺序不能乱：没折行算出的高度是错的，clamp 会按错高度裁；重叠检测又依赖 clamp 后的最终坐标。**收尾再夹一次**：overlap / card-height / timeline 这些会纵向移动文字的 pass 跑完后，再调一次 `_clamp_text_baseline_y`——因为它们会把堆叠文字无上界地往下推出画布，而前面的 clamp 早已跑过、没人兜底（幂等，没推出去就是 no-op）。
2. **LLM 审阅 `review_and_fix_svg`**：受**成本门控**——没 warning 不审、warning 全是"已自动修好"的小问题也不审，只有出现真问题才花一次 LLM 调用。**它是单次 LLM pass，无渲染、无重试**（CLAUDE.md 旧称"cairosvg 直渲→审查→修→再生成迭代循环"已于 `0802781` 对齐为"单次 pass"）。**占位符保护**：审前审后比对 `__IMAGE_xxx__` 占位符计数，LLM 要是把图片占位弄丢了，直接丢弃审阅结果、退回审前 SVG。**审后再校验**：review 真跑过的页，紧接着**再跑一次 `validate_and_fix`**——reviewer 唯一的结构护栏只有占位符计数，可能重新引入越界元素/不安全字体，确定性校验把质量底线补回来。
3. **`sanitize_for_ppt`**：删 `<script>` / `on*` 事件 / emoji / 注释 / root 宽高，拍平嵌套 tspan，圆形序号的 y 吸附到圆心。**解析用的是和 validator 同一个 `_SAFE_PARSER`**（本轮修复前这里用的是默认 parser，XXE 硬化只覆盖了两条解析路径的一条）。
...
旧版 `materials/ai_image_asset_db.py` 是一个 8896 行的单体，扛建库/检索/入库/policy/review 全部职责，是公认的"维护性炸弹"。**Phase A 重构已把它绞杀成 19 个按职责切分的模块**（`edupptx/reuse/`，合计 9572 行），原文件退化为 **1126 行、0 自有定义的 re-export 文件**。

> ⚠️ **它不是"临时兼容 shim"，是永久的唯一公共门面**：全仓 23 个源文件（agent / cli / content_planner / strict_reuse_classifier / asset_ingest_worker / vlm_asset_enricher …）都从它 import，**0 个直接 import `reuse/` 子包**。也就是说重构只拆了内部实现、对外 API 面没变：调用方看不到 19 模块结构，每新增一个名字都要穿过它的 re-export 列表，且必须永远与子包保持字节级一致。把它当"迁移期可删的 shim"会误判——除非把 23 个调用方逐个改到直接 import `reuse/`，否则它是常驻门面。
...
### 🟡 文档与代码不符（CLAUDE.md 目录树，仅剩一项）
- **页型模板目录**：CLAUDE.md 目录树仍把页型参考模板列在 `design/style_templates/` 名下，未反映已搬到 `design/page_templates/{复用,低年级,高年级}/`。
...
**Phase 4 校验/输出（独立 re-audit 新发现并修复，原 §5 未列）：**
- ~~裸 `<` 转义只看下一个字符，`n<k` / `0<x<1` 让整页 `validate_and_fix` 静默退回原文、跳过全部修复~~ → 改**标签成形 lookahead**（P0，4 条回归测试，含 sanitizer 同源 bug）。
...
2. ~~8896 行复用核心是维护性炸弹~~ —— **已由 Phase A 重构解决**（19 模块子包；但 1126 行原文件是常驻公共门面而非临时 shim，见 §3.1，对外 API 面未变是遗留代价）。
